...
class Stack(ttk.LabelFrame):
    ...
    def __init__(
        self,
        parent,
        text=f"Stack {stack_n}",
        borderwidth=4,
        width=300,
        height=200,
        columns=[("ID", 20, "center"), ("Value", 50, "center")],
        selectmode="none",
        values=[],
        index=0,
    ):
        super().__init__(
            parent, text=text, borderwidth=borderwidth, width=width, height=height
        )
        ...
        self.tw = ttk.Treeview(
            ...
            yscrollcommand=self.twscroll.set,
...
class StackEngineer:
    ...
    def run(self):
        ...
        self.api.register_game(StackEngineerGame)


def setup(api):
    api.register("stackengineer", StackEngineer(api))
