    ...
    def run(self):
        class TetrisGame(self.api.Game):
            ...
            def clear_lines(self):
                ...
                all_squares_h = dict(
                    zip(all_squares, [self.board.coords(sq)[3] for sq in all_squares])
                )
                count = Counter()
                for sq in all_squares_h.values():
                    count[sq] += 1
        ...
        self.api.register_game(TetrisGame)


def setup(api):
    api.register("tetris", Tetris(api))
