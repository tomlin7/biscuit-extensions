    ...
    def run(self):
        class TypoGame(self.api.Game):
            ...
            def __init__(self, master, *args, **kwargs):
                ...
                self.pos = 0
                self.dict = (
                    "LOREM IPSUM DOLOR SIT AMET cats and dogs are awesome".split()
                )
            ...
            def randomize(self, *_):
                self.words = [
                    self.dict[random.randint(0, len(self.dict) - 1)]
                    for _ in range(0, len(self.dict))
                ]
        ...
        self.api.register_game(TypoGame)


def setup(api):
    api.register("typo", Typo(api))
